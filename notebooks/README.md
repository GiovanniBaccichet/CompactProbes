...
This notebook is an attempt to obtain proper embeddings to represent the distance between different features, with respect to the Probe Requests' Information Elements.

This attempt is prior to the development of the custom dissector for Information Elements subfields, that allows for a proper comparison between different fields (since sub-fields are binary or numeric, while the complete field is a hex map).

### IE_one-hot_encoding

> Input: `balanced_df_raw.csv`

> Output: `encoded_ONE-HOT_balanced_df.csv`

This notebook performs one-hot encoding to the dataset in input. Output data is then used for data exploration and data visualization.

### IE_sum_encoding

> Input: `balanced_df_raw.csv`

> Output: `encoded_SUM_balanced_df.csv`

This notebook performs SUM encoding for each feature extracted as a HEX map: each feature is equal to the sum of the ASCII characters that compose the HEX map. This is the same encoding used in [Pintor et al.](https://ieeexplore.ieee.org/document/10001618). In this notebook we also **test** the possible **collisions** that are generated by the mapping, noticing that there are in fact several of them.
